...
def solve_DE(deg,deg_out,xs,ys,fs,lam=1):
    ...
    Mx3 = M_x_power(deg, 3, deg_out=deg_out)
    ...
    ## l calculation
    term1 = [Mx3@Dx,Mx2@Dy,M1,M1]
    term2 = [Mx1@Dx,Mx2@Dy,M1,M1]
    term3 = [Mx3@Dx,M1@Dy,M1,M1]
    term4 = [Mx1@Dx,M1@Dy,M1,M1]
    l = (constract_iota_state(ITn,term1)*50-
         constract_iota_state(ITn,term2)*36-
         constract_iota_state(ITn,term3)*32+
         constract_iota_state(ITn,term4)*18)/fs
...
if __name__ == "__main__":
    ...
    # True solution
    def true_f(x,y):
        return x*(1-x*x)*(1-y*y)

    # Solver and problem parameters
    n = 5
    deg = 2**n-1
    deg_out = 2**(n+1)-1
    xs, ys, fs = 0.5, 0, true_f(0.5,0)
    lam = 1000
