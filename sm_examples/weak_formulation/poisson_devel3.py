...
def solve_Poisson(deg,boundaries,xs,ys,lam=1):
    ...
    v = ITn @ (np.kron(GTn,GTn) - 0.5*np.kron(Dn,In))
    ...
    tau_p1 = encoding.chebyshev_encoding(deg=deg, x=boundaries[1][0]) @ GTn
...
if __name__ == "__main__":
    ...
    def true_f(x):
        return 0.75 + 0.5*x - 0.25*x*x

    n = 2
    deg = 2**n-1
    xs, ys = 0, true_f(0)
    boundaries = ((-1.0,0.0),(1.0,0.0))
    lam = 1000
    ...
    plt.plot(x_plot, true_f_plot, label='true_f')
    plt.plot(x_plot, appr_f_plot, label='appr_f')
