...
def solve_DE(deg,deg_out,xs,ys,fs,lam):
    ...
    for m in range(deg+1):
        for k in range(deg//2+1):
            Mxm1 = M_x_power(deg, m+1, deg_out=deg_out)
            Mxm2 = M_x_power(deg, m, deg_out=deg_out)
            Myk = M_x_power(deg, 2*k+1, deg_out=deg_out)
            term1 = [Mxm1@Dx,Myk@Dy,M1,M1]
            term2 = [Mxm2@Dx,Myk@Dy,M1,M1]
            c1 = 0.2*(-1)**k/(factorial(m)*factorial(2*k+1))
            c2 = 0.2*(-1)**k/(factorial(m)*factorial(2*k))
            l += (constract_iota_state(ITn,term1)*c1+
                  constract_iota_state(ITn,term2)*c2)
    l /= fs
    ...
    g_sin = sin_cheb_appr(deg)
    g_sin[0] *= np.sqrt(2)
    c1 = 0.2*np.exp(-1)*np.sin(1)/np.dot(chebyshev_encoding(deg,1),g_sin)
    g1 = c1*g_sin
    c2 = 0.2*np.exp(1)*np.sin(1)/np.dot(chebyshev_encoding(deg,1),g_sin)
    g2 = c2*g_sin

    g_exp = exp_cheb_appr(deg)
    g_exp[0] *= np.sqrt(2)
    c3 = 0.2*np.sin(-1)/np.dot(chebyshev_encoding(deg,0),g_exp)
    g3 = c3*g_exp
...
if __name__ == "__main__":
    ...
    # True solution
    def true_f(x,y):
        return 0.2*np.exp(x)*np.sin(y)

    # Solver and problem parameters
    n = 4
    deg = 2**n-1
    deg_out = 2**(n+1)-1
    xs, ys, fs = 0.5, 0.5, true_f(0.5,0.5)
    lam = 1000
